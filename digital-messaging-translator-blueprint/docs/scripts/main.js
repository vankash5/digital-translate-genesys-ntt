...
async function onMessage(data) {
    ...
    if (messageType === 'chat') {
        ...
        case 'message':
        {
            ...
            const translatedData = await translate.translateText(message, customerLanguage, genesysCloudLanguage);
        ...
        if (data.topicName.toLowerCase() === 'channel.metadata') {
            ...
            data.eventBody.participants.forEach((participant) => {
                if (!participant.endTime) {
                    try {
                        ...
                        console.error('Wrong participant format', participant);
                    }
                }
            });
            ...
            if (publish) {
                const messageDetail = await conversationsApi.getConversationsMessageMessage(data.eventBody.id, messageId);
                // Make sure message is published only once
                if (!messageIds.includes(messageId)) {
                    messageIds.push(messageId);

                    // Wait for translate to finish before calling addChatMessage
                    const translatedData = await translate.translateText(messageDetail.textBody, customerLanguage, genesysCloudLanguage);
                    view.addChatMessage(name, translatedData.translated_text, purpose);
                }
            }
        }
...
async function sendChat() {
    ...
    const translatedData = await translate.translateText(message, genesysCloudLanguage, customerLanguage);
...
async function translateAndDisplay(displayItems) {
    const preparedForTranslation = displayItems.map(({ message }) => ({
        text: message,
        sourceLanguage: customerLanguage,
        targetLanguage: genesysCloudLanguage
    }));
    const translatedItems = await translate.translateTextBatch(preparedForTranslation);
    translatedItems.forEach((translatedItem, i) => {
        view.addChatMessage(displayItems[i].name, translatedItem.translated_text, displayItems[i].purpose);
    });
}
 ...
 * @returns {Promise} 
 */
async function showChatTranscript(conversationId) {
    if (messageType === 'chat') {
        const data = await conversationsApi.getConversationsChatMessages(conversationId);
        // Show each message
        const displayItems = [];
        for (const msg of data.entities) {
            ...
            displayItems.push({ name, purpose, message });
        }
        await translateAndDisplay(displayItems);
    } else if (messageType === 'message') {
        let data = await conversationsApi.getConversation(conversationId);
        ...
        data = await conversationsApi.postConversationsMessageMessagesBulk(conversationId, { 'body': messageIds });
        data.entities.reverse();
        const displayItems = data.entities.map((msg) => {
 ...
 * @returns {Promise}
 */
async function setupChatChannel(conversationId) {
...
genesysCloudLanguage = urlParams.get('language') || 'en-us';
...
client.loginImplicitGrant(
    ...
    .then(data => {
        ...
        if (null != customer.chats && customer.chats.length > 0) {
            ...
            messageType = 'message';
            //NTT custom fields
            customerName = (customer.attributes.fullname || 'Customer not recognized');
            customerLanguage = customer.attributes.language || customer.attributes.browser_language || 'auto';
        }
